...
Requested role: deployer, API/indexer operator, or replay verifier
...
The output we need is a public operator attestation JSON file with your operator id, contact or public key, organization or independent-individual marker, independence statement, tested commit, chain/RPC details, and role-specific checks.

This does not require a perfect launch. If you find issues, record them in the attestation observations and we will include them in the launch summary.

## Civic Or Community Steward
...
Would you be willing to run a community-steward role during the testnet window?

Roster slot: steward-slot-here
Runbook: `docs/public-testnet-operator-runbook.md`
Assignment intake: `docs/public-testnet-operator-assignment-intake.md`

The steward role focuses on governance-process review, emergency/safety drill checks, and a public attestation explaining what was tested. We will also ask for a contact or public key and a short independence statement.
...
Would you be willing to run an API/indexer or replay-verifier role?

Requested role: api-indexer or replay-verifier
Roster slot: slot-id-here
Issue body: `docs/public-testnet-operator-issue-bodies/slot-id-here.md`

The key output is a `public-testnet-operator-attestation-v0` JSON file. Replay verifiers should independently report matching transaction and event stream hashes for the same testnet window. API/indexer operators should expose a public API base URL and publish replay hashes.
...
5. Do not mark the slot reviewed until a valid attestation exists and maintainer independence review passes.

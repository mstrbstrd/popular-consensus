...
Rehearsal evidence must not be used to close the final roadmap item. The final gate requires real independent operators, public evidence files, maintainer independence review, and `pnpm testnet:verify-attestations` reporting `Ready` against the real evidence directory.

## Rehearsal Goals

- Confirm maintainers understand the operator roster flow.
- Confirm each role can generate a correctly shaped evidence file.
...
Do not write rehearsal evidence files to `docs/public-testnet-attestations`.

## Rehearsal Steps

1. Pick one maintainer to play each public-testnet role.
2. Use `docs/public-testnet-role-command-reference.md` to generate temporary evidence files with manual hashes.
...
The rehearsal is successful only if maintainers understand the process and the real MVP gate remains open. Do not copy rehearsal evidence files or rehearsal launch summaries into the real evidence paths.
